...
<body class="bg-gray-50 min-h-screen py-12 flex items-center justify-center relative">
                        ...
                        <div>
                            ...
                            <div class="flex gap-6">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="gender" value="L" checked class="w-5 h-5 text-purple-600 focus:ring-purple-500">
                                    <span class="font-medium text-gray-700">Laki-laki</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="gender" value="P" class="w-5 h-5 text-purple-600 focus:ring-purple-500">
    ...
    <script>
        ...
        function showError(field, message) {
            ...
            $(`#error-${field}`).text(message).addClass('show');
        }
        ...
        function validateForm() {
            ...
            const jenis_kelamin = $('input[name="gender"]:checked').val();
            ...
            // Validate no_hp
            if (!no_hp) {
                showError('no_hp', 'No. Handphone wajib diisi!');
                isValid = false;
            } else if (!/^08[0-9]{8,11}$/.test(no_hp)) {
                showError('no_hp', 'Format nomor HP tidak valid! (08xxxxxxxxxx)');
                isValid = false;
            }
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: response.message || 'Pendaftaran berhasil! Silakan login.',
                            showConfirmButton: true,
                            confirmButtonColor: "#667eea",
                            confirmButtonText: 'OK'
                        }).then(() => {
                        //     // Redirect ke halaman login
                            window.location.href = response.redirect || '/customer-login';
                        });
                    } else {
                        // Reset button
                        btnRegister.html(originalBtnText).prop('disabled', false);
                        showAlert('error', response.message || 'Terjadi kesalahan saat mendaftar!');
                    }
                },
                error: function(xhr) {
                    // Reset button
                    ...
                    if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                        ...
                        Object.keys(errors).forEach(field => {
                            ...
                            showError(fieldName, errors[field][0]);
                        });

                        showAlert('error', 'Mohon periksa kembali form Anda!');
                    } else {
