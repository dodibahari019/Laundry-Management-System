...
function updateRecentOrders(orders) {
    ...
    if (orders.length === 0) {
        ...
        orders.slice(0, 5).forEach(x => {
            ...
            // Ambil layanan pertama dari order items
            // let firstItem = x.order_items && x.order_items.length > 0 ? x.order_items[0] : null;
            // let namaLayanan = firstItem && firstItem.layanan ? firstItem.layanan.nama_layanan : '-';
            // let jenisLayanan = firstItem && firstItem.layanan ? firstItem.layanan.jenis : '-';
            // let jenisColor = jenisLayanan === 'kiloan' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700';
            
            let layananText = '-';
            
            if (x.order_items && x.order_items.length > 0) {
                let firstItem = x.order_items[0];
                let nama = firstItem.layanan?.nama_layanan ?? '-';
                let jenis = firstItem.layanan?.jenis ?? '-';
                let sisa = x.order_items.length - 1;

                let jenisColor = jenis === 'kiloan'
                    ? 'bg-blue-100 text-blue-700'
                    : 'bg-orange-100 text-orange-700';

                layananText = `
                    <p class="text-sm text-gray-900">${nama}</p>
                    <span class="${jenisColor} px-2 py-0.5 rounded text-xs font-semibold">
                        ${jenis}
                    </span>
                `;
