...
@endsection
@section('HeaderTitle', 'Kelola User')
@section('Description', 'Kelola semua User yang bisa masuk dalam aplikasi laundry')
    ...
    <div class="bg-white rounded-2xl p-6 mb-6 border border-gray-100">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    <input id="searchInput" type="text" placeholder="Cari username, nama, email..." class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-100 focus:border-purple-500">
            ...
            <div class="flex items-end">
                <button data-url="/users/create" class="modal-crud w-full px-4 py-3 gradient-primary text-white rounded-xl font-semibold hover:shadow-lg transition flex items-center justify-center space-x-2">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <div>
                ...
                <p id="jumlahUsersInfo" class="text-sm text-gray-500 mt-1">Menampilkan {{ $jumlahSemua }} Users</p>
            </div>
        ...
        <div class="p-6 border-t border-gray-200 flex items-center justify-between">
            ...
            <div id="paginationContainerUsers" class="flex space-x-2">
                ...
                @else
                    <button data-page="{{ $dataUsers->currentPage() - 1 }}" class="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Previous</button>
                @endif

                @foreach ($dataUsers->getUrlRange(1, $dataUsers->lastPage()) as $page => $url)
                    <button data-page="{{ $page }}" class="px-3 py-2 rounded-lg text-sm font-semibold {{ $dataUsers->currentPage() == $page ? 'bg-purple-600 text-white' : 'border text-gray-600 hover:bg-gray-50' }}">
                        {{ $page }}
                    </button>
                @endforeach

                @if($dataUsers->hasMorePages())
                    <button data-page="{{ $dataUsers->currentPage() + 1 }}" class="px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Next</button>
                @else
...
<script>
    function updateTableUsers(res) {
        ...
        if (res.data && res.data.length > 0) {
            res.data.forEach((x, index) => {
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="py-4 px-6 font-semibold">${nomor}</td>
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center justify-center space-x-2">
                            ...
                            <button onclick="confirmDeleteUsers('${x.id_user}')"" class="text-red-600 hover:text-red-800 p-1" title="Hapus">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        $('#idBodyTableUsers').html(html);
    }

    function updateInfoUsers(res) {
        // Update jumlah layanan
        $('#jumlahUsersInfo').html(
            `Menampilkan ${res.total} Users`
        );
