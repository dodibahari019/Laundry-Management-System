...
<body>
    ...
    <div class="info-box">
        ...
        <div class="info-row">
            ...
            <div class="info-value">: {{ count($data) }} pelanggan</div>
        </div>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 20%;" class="text-right">Rata-Rata Belanja</th>
                <th style="width: 20%;" class="text-center">Transaksi Terakhir</th>
            </tr>
        </thead>
        <tbody>
            @php
            ...
            <tr>
                ...
                <td>
                    <div class="font-bold">{{ $item->nama_pelanggan }}</div>
                    <span class="small-text">{{ $item->no_hp }}</span>
                </td>
                <td class="text-right font-bold">{{ number_format($item->jumlah_transaksi, 0, ',', '.') }}</td>
                <td class="text-right font-bold">Rp {{ number_format($item->total_belanja, 0, ',', '.') }}</td>
                <td class="text-right font-bold">Rp {{ number_format($item->rata_rata_belanja, 0, ',', '.') }}</td>
                <td class="text-center">{{ \Carbon\Carbon::parse($item->tanggal_transaksi_terakhir)->format('d/m/Y') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="summary-box">
        <div class="summary-row">
            ...
            <span class="summary-value">{{ count($data) }} pelanggan</span>
        </div>
        <div class="summary-row">
        ...
        <div class="summary-row">
            ...
            <span class="summary-value">Rp {{ number_format($totalBelanja, 0, ',', '.') }}</span>
        </div>
    </div>
</body>
</html>
